...
const realWorkerRuntimePath = ".sandcastle/real-worker.mjs";
...
const realProviderAvailable = () => {
  try {
    ...
    const authentication = execFileSync("codex", ["login", "status"], {
      encoding: "utf8",
      env: process.env,
      timeout: 5_000,
    }).trim();
    ...
    return version === `codex-cli ${codexVersion}` && /^Logged in\b/.test(authentication);
  } catch {
    return false;
  }
};
...
const inspectRuntime = (retainedExecutionInputs = null) => {
  ...
  if (real) {
    ...
    const providerReady = hasExactKeys(real, ["schemaVersion", "provider", "scenario"])
      ...
      && workerEnvironment.skills.some(({ identity }) =>
...
const runWorker = async (execution, readiness) => {
  ...
  let workerExit = { code: null, startFailed: dependencyFailure };
  const results = [];
