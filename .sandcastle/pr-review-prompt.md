...
5. **Verify each criterion is satisfied through the real product path.** If a criterion is demonstrated only because the test harness staged a precondition that no shipped source code ever creates — a manifest, fixture file, built image, or environment variable — the requirement is **not met**, however green the test is. Ask of each passing acceptance test: could a user reach this through the software's own public interface, on a clean install, without a test fixture preparing the ground? If not, block.
6. Reject internal-API-only evidence for a requirement stated in terms of a user-facing surface, test doubles standing in for real provider or process requirements, fabricated person approvals, weakened tests, secrets, or prohibited side effects.
