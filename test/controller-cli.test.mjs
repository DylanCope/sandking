...
test("sandking launch invokes one ordinary Controller CLI operation", async () => {
  ...
  try {
    ...
    assert.equal(requests[0].operation, "harness-run.launch");
    ...
    assert.doesNotMatch(JSON.stringify(requests[0]), /approve|prepare|plugin|skill/i);
  } finally {
