...
import {
  ...
  verifyIssue121EvidenceSourceRevisionUnchanged,
...
if (
  ...
  throw new Error("issue_121_acceptance_manifest_invalid");
}

const repositoryRoot = process.cwd();
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const evidenceSourceRevision = updateEvidence
  ? await captureIssue121EvidenceSourceRevision({
      repositoryRoot,
      demonstratedPaths: ISSUE_121_DEMONSTRATED_PATHS,
    })
  : null;
const resultDirectory = await mkdtemp(join(tmpdir(), "sandking-reconnect-evidence-"));
const observationPath = join(resultDirectory, "canonical-reconnect-browser-observation.json");
...
try {
  ...
  if (updateEvidence) {
    await verifyIssue121EvidenceSourceRevisionUnchanged({
      repositoryRoot,
      demonstratedPaths: ISSUE_121_DEMONSTRATED_PATHS,
    ...
    if (observation.scenario !== manifest.scenarios[0].id || observation.issue !== 121) {
      throw new Error("issue_121_browser_observation_invalid");
    }
    ...
    if (
      ...
      throw new Error("issue_121_evidence_not_sanitized");
    }
    const evidencePath = resolve(dirname(manifestPath), "evidence", "issue-121.json");
