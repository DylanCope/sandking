...
import {
  ...
  verifyIssue118EvidenceSourceRevisionUnchanged,
...
const evidenceSourceRevision = updateEvidence
  ? await captureIssue118EvidenceSourceRevision({
    repositoryRoot,
    demonstratedPaths: ISSUE_118_DEMONSTRATED_PATHS,
  })
  : null;
const resultDirectory = await mkdtemp(join(tmpdir(), "sandking-project-evidence-"));
const observationPath = join(resultDirectory, "project-browser-observation.json");
...
try {
  ...
  if (updateEvidence) {
    await verifyIssue118EvidenceSourceRevisionUnchanged({
      repositoryRoot,
      demonstratedPaths: ISSUE_118_DEMONSTRATED_PATHS,
      expectedRevision: evidenceSourceRevision,
    });
    const [observation, pathResolution, mutationContract] = await Promise.all([
      readFile(observationPath, "utf8").then(JSON.parse),
      readFile(join(resultDirectory, "project-path-resolution.json"), "utf8")
        .then(JSON.parse),
      readFile(join(resultDirectory, "project-mutation-contract.json"), "utf8")
        .then(JSON.parse),
    ]);
    if (observation.scenario !== manifest.scenarios[0].id) {
      throw new Error("issue_118_browser_observation_invalid");
    }
    ...
    if (
      /project-browser-secret|controller-secret-must-not-reach-host|bootstrap\?token=|sandking_session=/i
        .test(evidenceText)
      || pathResolution.secretFixtureRetained
    ) {
      throw new Error("issue_118_evidence_not_sanitized");
    }
    const evidencePath = resolve(dirname(manifestPath), "evidence", "issue-118.json");
