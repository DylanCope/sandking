...
test("stale Harness-run state fails with typed reset guidance without rewriting it", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "sandking-stale-harness-run-state-"));
  try {
    for (const staleState of [
      ...
      await writeFile(join(dataDir, "harness-runs.json"), staleStateText);
      await assert.rejects(
        createHarnessRunManager({
          dataDir,
          hostId,
          recordAudit: async () => {
            throw new Error("stale_state_must_not_publish_audits");
          },
          loadLaunchContext: async () => {
            throw new Error("stale_state_must_not_load_launch_context");
          },
        }),
        (error) => {
          assert.ok(error instanceof HarnessRunStateError);
          assert.equal(error.code, "harness_run_state_schema_unsupported");
          assert.equal(error.stateDirectory, dataDir);
          assert.match(error.message, /delete the Sand-King state directory/i);
          return true;
        },
      );
      assert.equal(await readFile(join(dataDir, "harness-runs.json"), "utf8"), staleStateText);
    }
  } finally {
